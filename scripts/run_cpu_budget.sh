...
set -uo pipefail
cd "$(dirname "$0")/.."
DATA=/home/user/data; WORK=/home/user/work/squad; RUNS=/home/user/work/runs
BB=$DATA/models/distilbert-base-uncased
COMMON="--data $WORK --backbone $BB --d_len 192 --l1_d 5e-4 --batch_size 8"
log() { echo "[$(date +%H:%M:%S)] $*"; }
...
log "waiting for teacher"; while pgrep -f "dyvo.teacher" >/dev/null; do sleep 20; done
